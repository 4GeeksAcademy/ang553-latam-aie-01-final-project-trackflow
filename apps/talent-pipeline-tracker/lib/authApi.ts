/**
 * Auth API client for Talent Pipeline Tracker.
 *
 * Provides functions for login, register, current user retrieval,
 * and profile management against the shared TrackFlow backend.
 *
 * @remarks
 * - Uses ``authFetch`` (from ``@/lib/authFetch``) for all
 *   authenticated endpoints — it automatically injects the
 *   ``Bearer`` token and removes it on ``401``.
 * - Uses plain ``fetch`` for public endpoints (login, register).
